...
- With LAN Developer Mode enabled, authenticated MQTT/TLS on `device/{serial}/report` and `device/{serial}/request` returned a real active-print snapshot from the P1S, and `auto_print.py status` reproduced that state after the full-snapshot fix. Authenticated implicit FTPS uploaded the generated sliced project as `codex_gear_validation.3mf`. Real pause/resume commands restored the active user-started job to `printing`. The latest snapshot reports `print_error=0` and HMS `0300310000010001` (part-cooling-fan speed/stall). HMS `0500050000010007` appeared transiently after a redundant retry and cleared from a later snapshot. Because the user-started print occupies the plate, no generated-model start or stop command was sent. Bambu Lab describes Developer Mode MQTT/FTP as unsupported interfaces, so firmware changes are a compatibility risk.
