...
`BAMBU_SLICER_COMMAND` and `BAMBU_SLICER_OUTPUT_EXT` are optional local hooks for a verified external slicer. Tests cover command success/failure and still validate the generated output before queueing; real slicer syntax and output quality remain an external gate.
