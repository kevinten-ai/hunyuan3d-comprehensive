...
`scripts/ai_to_print.py` returns nonzero when generation does not produce a model, while `--mock --no-print` remains the local success demo path. Before automatic queueing, it converts source model files to queue-ready 3MF.

`scripts/continuous_print.py generate` returns nonzero when required inputs are missing or generation does not produce a model; `--mock --no-print` remains the local success demo path. Before automatic queueing, it converts source model files to queue-ready 3MF.
